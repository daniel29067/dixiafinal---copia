...
public class IbrevaScript extends BasicScript implements PhysicsContact {
    ...
    public IbrevaScript(PooledEngine engine) {
        mEngine = engine;
    }
    ...
    public void init(Entity item) {
        ...
        animEntity = itemWrapper.getChild("ibreva1").getEntity();
    ...
    public IbrevaComponent getPlayerComponent() {
        return ComponentRetriever.get(animEntity, IbrevaComponent.class);
    }
    ...
    public void beginContact(Entity contactEntity, Fixture contactFixture, Fixture ownFixture, Contact contact) {
        ...
        IbrevaComponent playerComponent = ComponentRetriever.get(animEntity, IbrevaComponent.class);

        if (mainItemComponent.tags.contains("lineas"))
            playerComponent.touchedPlatforms++;

        Palabrabre1Component palabrabre = ComponentRetriever.get(contactEntity, Palabrabre1Component.class);
        if (palabrabre != null) {
            playerComponent.touchedWords += palabrabre.value;
