...
public class IbrochaScript  extends BasicScript implements PhysicsContact {
    ...
    public IbrochaScript(PooledEngine engine) {
        mEngine = engine;
    }
    ...
    public void init(Entity item) {
        ...
        animEntity = itemWrapper.getChild("ibrocha").getEntity();
    ...
    public IbrochaComponent getPlayerComponent() {
        return ComponentRetriever.get(animEntity, IbrochaComponent.class);
    }
    ...
    public void beginContact(Entity contactEntity, Fixture contactFixture, Fixture ownFixture, Contact contact) {
        ...
        IbrochaComponent playerComponent = ComponentRetriever.get(animEntity, IbrochaComponent.class);

        if (mainItemComponent.tags.contains("lineas"))
            playerComponent.touchedPlatforms++;

        Palabrabro1Component palabrabro = ComponentRetriever.get(contactEntity, Palabrabro1Component.class);
        if (palabrabro != null) {
            playerComponent.touchedWords += palabrabro.value;
