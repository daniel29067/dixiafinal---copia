...
public class IbrincoScript extends BasicScript implements PhysicsContact {
    ...
    public IbrincoScript(PooledEngine engine) {
        mEngine = engine;
    }
    ...
    public void init(Entity item) {
        ...
        animEntity = itemWrapper.getChild("ibrinco").getEntity();
    ...
    public IbrincoComponent getPlayerComponent() {
        return ComponentRetriever.get(animEntity, IbrincoComponent.class);
    }
    ...
    public void beginContact(Entity contactEntity, Fixture contactFixture, Fixture ownFixture, Contact contact) {
        ...
        IbrincoComponent playerComponent = ComponentRetriever.get(animEntity, IbrincoComponent.class);

        if (mainItemComponent.tags.contains("lineas"))
            playerComponent.touchedPlatforms++;

        Palabrabri1Component palabrabri = ComponentRetriever.get(contactEntity, Palabrabri1Component.class);
        if (palabrabri != null) {
            playerComponent.touchedWords += palabrabri.value;
