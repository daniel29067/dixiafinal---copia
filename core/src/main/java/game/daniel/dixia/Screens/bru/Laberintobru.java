...
public class Laberintobru implements Screen {


    private AssetManager mAssetManager;
    private IbrujaComponent ibrujaComponent;
    ...
    private HUDlaberintobru mHUD;
    private ExtendViewport mHUDViewport;

    public Laberintobru ( AsyncResourceManager mAsyncResourceManager, SceneLoader mSceneLoader, Viewport mViewport, AssetManager mAssetManager, OrthographicCamera mCamera) {
    ...
    public void show() {
        ...
        ComponentRetriever.addMapper(IbrujaComponent.class);
        ComponentRetriever.addMapper(Palabrabro1Component.class);

        mCamera = new OrthographicCamera();
        mViewport = new ExtendViewport(15, 8, mCamera);

        mSceneLoader.loadScene("laberintobru", mViewport);

        ItemWrapper root = new ItemWrapper(mSceneLoader.getRoot());

        ItemWrapper player = root.getChild("ibruja1");
        player.getChild("ibruja1").getEntity().add(mEngine.createComponent(IbrujaComponent.class));
        IbrujaScript ibrujaScript= new IbrujaScript(mEngine);
        player.addScript(ibrujaScript, mEngine);
        cameraSystem.setFocus(player.getEntity());



        mSceneLoader.addComponentByTagName("palabrabru1", Palabrabra1Component.class);

        mHUDViewport = new ExtendViewport(768, 576);
        mHUD = new HUDlaberintobru(mAssetManager.get("skin/skin.json"), mAsyncResourceManager.getTextureAtlas("main"), mHUDViewport, mSceneLoader.getBatch());
        mHUD.setPlayerScript(ibrujaScript);
