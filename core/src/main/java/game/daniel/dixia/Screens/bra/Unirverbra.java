...
public class Unirverbra implements Screen {
    ...
    private HUDunirverbra mHUD;
    private ExtendViewport mHUDViewport;

    public Unirverbra(AsyncResourceManager mAsyncResourceManager, SceneLoader mSceneLoader, Viewport mViewport, AssetManager mAssetManager, OrthographicCamera mCamera) {
    ...
    public void show() {
        ...
        ComponentRetriever.addMapper(Pbrazo1Component.class);

        mCamera = new OrthographicCamera();
        mViewport = new ExtendViewport(15, 8, mCamera);

        mSceneLoader.loadScene("unirverbra", mViewport);

        ItemWrapper root = new ItemWrapper(mSceneLoader.getRoot());

        ItemWrapper player = root.getChild("pbrazo1");
        player.getChild("brazo2").getEntity().add(mEngine.createComponent(Pbrazo1Component.class));
        PbrazoScript pbrazoScript = new PbrazoScript(mEngine);
        player.addScript(pbrazoScript, mEngine);
        cameraSystem.setFocus(player.getEntity());


        mSceneLoader.addComponentByTagName("palabrabra1", Palabrabra1Component.class);
        mSceneLoader.addComponentByTagName("palabrabra2", Palabrabra2Component.class);
        mSceneLoader.addComponentByTagName("palabrabra3", Palabrabra3Component.class);
        mSceneLoader.addComponentByTagName("palabrabra4", Palabrabra4Component.class);
        mSceneLoader.addComponentByTagName("palabrabra5", Palabrabra5Component.class);

        mHUDViewport = new ExtendViewport(768, 576);
        mHUD = new HUDunirverbra(mAssetManager.get("skin/skin.json"), mAsyncResourceManager.getTextureAtlas("main"), mHUDViewport, mSceneLoader.getBatch());
        mHUD.setPbrazoScript(pbrazoScript);
