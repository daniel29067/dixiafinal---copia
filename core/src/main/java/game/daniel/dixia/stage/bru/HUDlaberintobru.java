...
public class HUDlaberintobru extends Stage {


    private Label mBonesLabel;

    private IbrujaScript mPlayerScript;
    ...
    public HUDlaberintobru(Skin skin, TextureAtlas atlas, Viewport viewport, Batch batch) {
        ...
        upButton.addListener(new ClickListener(){
            ...
            public void clicked(InputEvent event, float x, float y) {
                mPlayerScript.movePlayer(IbrujaScript.JUMP);
            }
        });
    ...
    public void setPlayerScript(IbrujaScript ibrujaScript) {
        mPlayerScript = ibrujaScript;
    }
    ...
    public void act(float delta) {
        ...
        if (leftClicked)
            mPlayerScript.movePlayer(IbrujaScript.LEFT);
        if (rightClicked)
            mPlayerScript.movePlayer(IbrujaScript.RIGHT);

        if (bones != mPlayerScript.getPlayerComponent().touchedWords) {
            bones = mPlayerScript.getPlayerComponent().touchedWords;
            mBonesLabel.setText("x" + bones);
        }
    }
}
